...
t1 = Sys.time()
sim_data_tau50_100_normfit_thetahat <- foreach(i=1:100,.packages=c("rjags","R2jags","foreach"),.verbose=T )%dopar%{
    set.seed(123)
...
save(sim_data_tau50_100_normfit_thetahat,file="sim_data_tau50_100_normfit_thetahat.Rdata")
...
setwd("/work/02784/myang3/QRJM_sim2/results/20150721/thetahat")
load(dir()[2])
post_par = lapply(sim_data_tau50_100_normfit_thetahat, as.data.frame)
### randomly choose 1k posterior samples out of 3k
sample1k = sample(seq(1:3e3), 1e3, replace=FALSE)
dput(sample1k, 'sample1k_dataqt50_meanfit.txt')
sample1k = dget('sample1k_dataqt50_meanfit.txt')
...
### 1-50 part
t1 = Sys.time()
RE_all100qt50_meanfit_1_50 = vector(length = 50, mode='list')
for (i in 1:50){
    RE_all100qt50_meanfit_1_50[[i]] = LMJM_pred_re(curt_data[i], post_par[i])
}
Sys.time()-t1
setwd("/work/02784/myang3/QRJM_sim2/results/20150721/pred_re")
save(RE_all100qt50_meanfit_1_50, file="RE_all100qt50_meanfit_1_50.Rdata")


### ------------------------ ###
### 51-100 part
t1 = Sys.time()
RE_all100qt50_meanfit_51_100 = vector(length = 50, mode='list')
for (i in 51:100){
    RE_all100qt50_meanfit_1_50[[i-30]] = LMJM_pred_re(curt_data[i], post_par[i])
}
Sys.time()-t1
setwd("/work/02784/myang3/QRJM_sim2/results/20150721/pred_re")
save(RE_all100qt50_meanfit_1_50, file="RE_all100qt50_meanfit_1_50.Rdata")
...
RE_all100qt50_meanfit = vector(length=100, mode='list')
for (i in 1:50) RE_all100qt50_meanfit[[i]] = RE_all100qt50_meanfit_1_50[[i]]
for (i in 51:100) RE_all100qt50_meanfit[[i]] = RE_all100qt50_meanfit_1_50[[i-50]]
setwd('/work/02784/myang3/QRJM_sim2/results/20150721/pred_re')
save(RE_all100qt50_meanfit, file="RE_all100qt50_meanfit.Rdata")


pre_surv_LMJM_025_025 = pred_surv_LMJM(data=curt_data, par=post_par, pred_re=RE_all100qt50_meanfit, t=0.25, dt=0.25, gold_std=gold_std_res)
pre_surv_LMJM_025_1 = pred_surv_LMJM(data=curt_data, par=post_par, pred_re=RE_all100qt50_meanfit, t=0.25, dt=1, gold_std=gold_std_res_025_1)
pre_surv_LMJM_025_2 = pred_surv_LMJM(data=curt_data, par=post_par, pred_re=RE_all100qt50_meanfit, t=0.25, dt=2, gold_std=gold_std_res_025_2)

QRJM_pred_qt50data_meanfit = list(pre_surv_QRJM_025_025, pre_surv_QRJM_025_1, pre_surv_QRJM_025_2)
setwd("/work/02784/myang3/QRJM_sim2/results/20150721/pred_re")
save(QRJM_pred_qt50data_meanfit, file='QRJM_pred_qt50data_meanfit.Rdata')
