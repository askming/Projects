...
linear.covariate <- data.frame(mu = as.factor(c(rep(1, ng), rep(2, ns))),
                               ...
                               surv.time = c(rep(0, ng), data2$time)
                               )
...
### additional for model X and XII from discussion group ###
...
  data = joint.data, verbose=TRUE, control.compute=list(dic=TRUE))
# Note, for model XII I had to create U13, which is identical to U12, and U23 identical to U22. Also surv.time are just the survival times (data2$time in the original code).
