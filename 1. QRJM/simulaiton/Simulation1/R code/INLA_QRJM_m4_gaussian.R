...
sim_longitudinal_data = function(survival_data=surdata, n=250, time=c(0, 0.25, 0.5, 0.75, 1, 3), beta=c(1,1), delta=c(1,1)){
	...
	# Y use gaussian error
	for (i in 1:length(Y)) {
		Y[i] = X[i,] %*% beta + U[i,] %*% c(1, time_long[i]) + rnorm(1)	
...
###############################################################
sim_multiple_data = function(N, n, sur_fun=sim_Ti, longi_fun=sim_longitudinal_data, alpha){
...
multi_data = sim_multiple_data(N=10, n=250, alpha=c(1,1,1))
